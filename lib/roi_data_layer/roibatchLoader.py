...
class roibatchLoader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        assert len(minibatch_db[0]["track_id"]) == len(
            np.unique(minibatch_db[0]["track_id"])
        ), "Cannot have >1 track with same id in same frame."
        assert len(minibatch_db[1]["track_id"]) == len(
            np.unique(minibatch_db[1]["track_id"])
        ), "Cannot have >1 track with same id in same frame."
        ...
        for ientry, entry in enumerate(minibatch_db):
            ...
            if self.training:
                ...
                if ratio < 1:
                    ...
                    padding_data.append(
                        torch.FloatTensor(
                            int(np.ceil(data_widths[ientry] / ratio)),
                            data_widths[ientry],
                            3,
                        ).zero_()
                    )
                    ...
                    padding_data.append(
                        torch.FloatTensor(
                            data_heights[ientry],
                            int(np.ceil(data_heights[ientry] * ratio)),
                            3,
                        ).zero_()
                    )
                ...
                not_keep = (gt_boxes[ientry][:, 0] == gt_boxes[ientry][:, 2]) | (
                    gt_boxes[ientry][:, 1] == gt_boxes[ientry][:, 3]
                )
                keep = torch.nonzero(not_keep == 0).view(-1)

                gt_boxes_padding.append(
                    torch.FloatTensor(
                        self.max_num_box, gt_boxes[ientry].size(1)
                    ).zero_()
                )
                if keep.numel() != 0:
                    ...
                    num_boxes.append(
                        torch.LongTensor(
                            [min(gt_boxes[ientry].size(0), self.max_num_box)]
                        ).cuda()
                    )
                    curr_num_boxes = int(num_boxes[ientry][0])
                    gt_boxes_padding[ientry][:curr_num_boxes, :] = gt_boxes[ientry][
                        :curr_num_boxes
                    ]
                else:
                ...
                padding_data[ientry] = (
                    padding_data[ientry].squeeze(0).permute(2, 0, 1).contiguous()
                )
            ...
            if _DEBUG:
                if self.training:
                    ...
                    self._plot_image(
                        padding_data[ientry].permute(0, 2, 3, 1),
            ...
            self._plot_image(
                data[ientry].permute(0, 2, 3, 1), gt_boxes[ientry], num_boxes[ientry]
            )
