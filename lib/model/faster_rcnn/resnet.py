...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        downsample=None,
        dilation=1,
        dilate_first_conv=False,
    ):
        super(Bottleneck, self).__init__()
        if dilate_first_conv:
            self.conv1 = nn.Conv2d(
                inplanes, planes, kernel_size=1, stride=stride, bias=False, dilation=1
            )
        else:
            self.conv1 = nn.Conv2d(
        ...
        self.conv3 = nn.Conv2d(
            planes, planes * 4, kernel_size=1, bias=False, dilation=1
        )
...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=1000):
        ...
        # a trous
        self.layer4 = self._make_dilation_layer(
    ...
    def _make_dilation_layer(self, block, planes, blocks, stride=1, dilation=1):
        ...
        for i in range(1, blocks):
            layers.append(
                block(self.inplanes, planes, dilation=dilation, dilate_first_conv=True)
            )
...
class resnet(_RFCN):
    def __init__(
        ...
        num_layers=101,
        pretrained=False,
        pretrained_rfcn=False,
        class_agnostic=False,
    ):
        self.model_path = "data/pretrained_model/res101.pth"
        self.model_rfcn_path = (
            "data/pretrained_model/rfcn_detect.pth"
        )  # trained on Imagenet VID+DET
    ...
    def _init_modules(self):
        ...
        # Fix blocks
        for p in self.RFCN_base[0].parameters():
            p.requires_grad = False
        for p in self.RFCN_base[1].parameters():
            p.requires_grad = False

        assert 0 <= cfg.RESNET.FIXED_BLOCKS < 4
        if cfg.RESNET.FIXED_BLOCKS >= 3:
            for p in self.RFCN_base[6].parameters():
                p.requires_grad = False
        if cfg.RESNET.FIXED_BLOCKS >= 2:
            for p in self.RFCN_base[5].parameters():
                p.requires_grad = False
        if cfg.RESNET.FIXED_BLOCKS >= 1:
            for p in self.RFCN_base[4].parameters():
                p.requires_grad = False
        ...
        self.RFCN_net = nn.Conv2d(
            2048, 512, kernel_size=3, padding=6, stride=1, dilation=6
        )
        ...
        if self.pretrained_rfcn == True:
            ...
            pretrained_rfcn_dict = {
                k: v for k, v in pretrained_rfcn_dict.items() if k in self.state_dict()
            }
            self.state_dict().update(pretrained_rfcn_dict)
            self.load_state_dict(pretrained_rfcn_dict)

        self.corr_bbox_net = nn.Conv2d(
            1051, 4 * self.n_reg_classes * 7 * 7, [1, 1], padding=0, stride=1
        )
