...
class _RFCN(nn.Module):
    ...
    def __init__(self, classes, class_agnostic):
        ...
        self.RFCN_psroi_cls_pool = _PSRoIPooling(
            cfg.POOLING_SIZE,
            cfg.POOLING_SIZE,
        ...
        self.RFCN_psroi_loc_pool = _PSRoIPooling(
            cfg.POOLING_SIZE,
            cfg.POOLING_SIZE,
            spatial_scale=1.0 / 16.0,
            group_size=7,
            output_dim=4 * self.n_reg_classes,
        )
        ...
        self.RFCN_cls_net = nn.Conv2d(
            512, self.n_classes * 7 * 7, [1, 1], padding=0, stride=1
        )
        nn.init.normal(self.RFCN_cls_net.weight.data, 0.0, 0.01)

        self.RFCN_bbox_net = nn.Conv2d(
            512, 4 * self.n_reg_classes * 7 * 7, [1, 1], padding=0, stride=1
        )
        ...
        self.conv3_corr_layer = Correlation(
            pad_size=8, kernel_size=1, max_displacement=8, stride1=2, stride2=2
        )
        self.conv4_corr_layer = Correlation(
            pad_size=8, kernel_size=1, max_displacement=8, stride1=1, stride2=1
        )
        self.conv5_corr_layer = Correlation(
            pad_size=8, kernel_size=1, max_displacement=8, stride1=1, stride2=1
        )
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes):
        ...
        for i_leg in range(num_legs):
            ...
            if self.training:
                leg_roi_data = self.RFCN_proposal_target(
                    rois[i_leg], gt_boxes[i_leg][:, :, :5], num_boxes[i_leg]
                )
                rois[
                    i_leg
                ], leg_rois_label, leg_rois_target, leg_rois_inside_ws, leg_rois_outside_ws = (
                    leg_roi_data
                )

                leg_rois_label = Variable(leg_rois_label.view(-1).long())
                leg_rois_target = Variable(
                    leg_rois_target.view(-1, leg_rois_target.size(2))
                )
                leg_rois_inside_ws = Variable(
                    leg_rois_inside_ws.view(-1, leg_rois_inside_ws.size(2))
                )
                leg_rois_outside_ws = Variable(
                    leg_rois_outside_ws.view(-1, leg_rois_outside_ws.size(2))
                )
            ...
            leg_pooled_cls_feat = self.RFCN_psroi_cls_pool(
                rfcn_cls[i_leg], rois[i_leg].view(-1, 5)
            )
            leg_pooled_loc_feat = self.RFCN_psroi_loc_pool(
                rfcn_bbox[i_leg], rois[i_leg].view(-1, 5)
            )
            ...
            if self.training and not self.class_agnostic:
                ...
                bbox_pred_view = bbox_pred[i_leg].view(
                    bbox_pred[i_leg].size(0), int(bbox_pred[i_leg].size(1) / 4), 4
                )
                bbox_pred_select = torch.gather(
                    bbox_pred_view,
                    1,
                    leg_rois_label.view(leg_rois_label.size(0), 1, 1).expand(
                        leg_rois_label.size(0), 1, 4
                    ),
                )
            ...
            if self.training:
                ...
                RFCN_loss_bbox[i_leg] = _smooth_l1_loss(
                    bbox_pred[i_leg],
                    leg_rois_target,
                    leg_rois_inside_ws,
                    leg_rois_outside_ws,
            ...
            bbox_pred[i_leg] = bbox_pred[i_leg].view(
                batch_size, rois[i_leg].size(1), -1
            )
        ...
        if self.training:
            ...
            tracking_rois, tracking_rois_label, tracking_rois_target, tracking_rois_inside_ws, tracking_rois_outside_ws = (
                tracking_roi_data
            )
            tracking_rois_label = Variable(tracking_rois_label.view(-1).long())
            tracking_rois_target = Variable(
                tracking_rois_target.view(-1, tracking_rois_target.size(2))
            )
            tracking_rois_inside_ws = Variable(
                tracking_rois_inside_ws.view(-1, tracking_rois_inside_ws.size(2))
            )
            tracking_rois_outside_ws = Variable(
                tracking_rois_outside_ws.view(-1, tracking_rois_outside_ws.size(2))
            )
        ...
        psroi_pooled_tracking_rois = self.RFCN_psroi_loc_pool(
            tracking_reg_coords, tracking_rois.contiguous().view(-1, 5)
        )
        ...
        if self.training:
            ...
            RFCN_tracking_loss_bbox = _smooth_l1_loss(
                tracking_pred,
