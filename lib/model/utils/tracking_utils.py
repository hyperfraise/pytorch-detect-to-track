...
class VideoPostProcessor(object):
    def __init__(self, pred_boxes, scores, pred_trk_boxes, classes):
        ...
        self._aboxes = np.ndarray(
            shape=(self.num_classes, self.num_frames), dtype=np.object
        )
        # self._box_inds = [[[] for _ in xrange(self.num_frames)] for _ in xrange(self.num_classes)]
        self._box_inds = np.ndarray(
            shape=(self.num_classes, self.num_frames), dtype=np.object
        )
        # self._track_boxes = [[[] for _ in xrange(self.num_frames)] for _ in xrange(self.num_classes)]
        self._track_boxes = np.ndarray(
            (self.num_classes, self.num_frames), dtype=np.object
        )
        ...
        for j in range(1, self.num_classes):
            self._aboxes[j], self._box_inds[j], self.CONF_THRESH[j] = self._keep_top_k(
                self._aboxes[j],
                self._box_inds[j],
                self.num_frames,
                self.max_per_set,
            ...
            if nonempty_frames.size(0) > 0:
                ...
                for j_pth in range(num_paths_c):
                    for k_frame in range(frameIdx.size(0) - 1):
                        ...
                        temp_box = torch.cat(
                            [
                                self._paths[c]["boxes"][j_pth][k_frame][:4].unsqueeze(
                                    0
                                ),
                                torch.cuda.FloatTensor(
                                    [self._paths[c]["scores"][j_pth][k_frame]]
                                ).unsqueeze(0),
                            ],
                            dim=1,
                        )
                        if self._track_boxes[c][f_idx] is None:
                            ...
                            self._track_boxes[c][f_idx] = torch.cat(
                                [self._track_boxes[c][f_idx], temp_box], dim=0
                            )
        print("Done!")
        return self._paths
    ...
    def _make_tubes(self, frameBoxes, max_per_image, box_voting, tracks_cell):
        ...
        paths = self._zero_jump_link(
            object_frames_boxes,
            object_frames_scores,
    ...
    def _zero_jump_link(
        self, frames_boxes, frames_scores, frames_boxes_idx, frames_trackedboxes
    ):
        """
         ---------------------------------------------------------
        ...
        while not (isempty_vertex != 0).any():
            ...
            for i in range(T):
                ...
                data_track_scores[i] = torch.cuda.FloatTensor(num_states, 1).fill_(
                    np.nan
                )
                ...
                boxes = frames_boxes[V[0]][
                    id, :
                ]  # get box coordinates from top box at frame V[0]
                scores = frames_scores[V[0]][
                    id
                ]  # get box score of top box from frame V[0]
                for j in range(T - 1):
                ...
                mean_top_scrs = torch.mean(
                    top_scores[: int(np.ceil(0.5 * len(top_scores)))]
                )
                ...
                smooth_scores = torch.from_numpy(
                    cv2.filter2D(scores.cpu().numpy(), -1, gfilter)
                ).cuda()
        ...
        paths = {
            "total_score": torch.stack(paths_total_score),
    ...
    def _score_of_edge(self, v1, v2):
        ...
        if v1["trackedboxes"] is not None and v2["trackedboxes"] is not None:
            ...
            # overlaps (N1, N2)
            overlap_ratio_1 = bbox_overlaps(
                v1["boxes"].contiguous(), v1["trackedboxes"][0]
            )
            overlap_ratio_2 = bbox_overlaps(
                v2["boxes"].contiguous(), v1["trackedboxes"][1]
            )
            track_score = torch.mm(
                torch.round(overlap_ratio_1), torch.round(overlap_ratio_2).t()
            )
            score[track_score > 0.0] += 1.0
            track_score = (track_score > 0.0).float()
        else:
    ...
    def _process_frame_pairs(self):
        ...
        for i_pair in range(self.pred_boxes.size(0) - 1):
            print(
                "Post-processing detections from t={} t+tau={}".format(
                    i_pair, i_pair + 1
                )
            )
            # Get scores in first frame of pair above 0.01 (ignore bg score)
            tracklet_score, tracklet_cls = torch.max(
                self.scores[i_pair, 0][:, 1:], dim=1
            )
            tracklets = torch.nonzero(tracklet_score > 0.01).view(-1)
            if tracklets.numel() > 0:
                if self._ascores_track[i_pair] is None:
                    self._ascores_track[i_pair] = self.scores[i_pair, 0][tracklets][
                        :, 1:
                    ]
                else:
                    self._ascores_track[i_pair] = torch.cat(
                        [
                            self._ascores_track[i_pair],
                ...
                if self._aboxes_track[i_pair, 0] is None:
                    self._aboxes_track[i_pair, 0] = self.pred_boxes[i_pair, 0][
                        tracklets
                    ]
                    self._aboxes_track[i_pair, 1] = self.pred_trk_boxes[i_pair][
                        tracklets
                    ]
                else:
                    self._aboxes_track[i_pair, 0] = torch.cat(
                        [
                            self._aboxes_track[i_pair, 0],
                            self.pred_boxes[i_pair, 0][tracklets],
                        ]
                    )
                    self._aboxes_track[i_pair, 1] = torch.cat(
                        [
                            self._aboxes_track[i_pair, 1],
            ...
            for i_leg in range(self.pred_boxes.size(1)):
                ...
                for i_cls in range(1, self.num_classes):  # skip background class
                    inds = torch.nonzero(
                        scores[:, i_cls] > self.CONF_THRESH[i_cls]
                    ).view(-1)
                    if inds.numel() > 0:
                        ...
                        if self._aboxes[i_cls, frameindex] is None:
                            ...
                            self._aboxes[i_cls, frameindex] = torch.cat(
                                [self._aboxes[i_cls, frameindex], cls_entry], dim=0
                            )
                        ...
                        if self._box_inds[i_cls, frameindex] is None:
                            ...
                            self._box_inds[i_cls, frameindex] = torch.cat(
                                [self._box_inds[i_cls, frameindex], inds]
                            )
                    else:
