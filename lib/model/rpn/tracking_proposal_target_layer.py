...
class _TrackingProposalTargetLayer(nn.Module):
    ...
    def _get_tracking_regression_labels_pytorch(
            ...
            for j_leg in range(i_leg + 1, n_twins):
                for i_batch in range(batch_size):
                    ...
                    if j_num_boxes == 0 or i_num_boxes == 0:
                        ...
                        X = twin2_trk_id_set.expand(
                            twin1_trk_id_set.size(0), twin2_trk_id_set.size(0)
                        ).t()
    ...
    def _sample_gt_rois_pytorch(
        self, gt_boxes, fg_rois_per_image, rois_per_image, num_classes, num_boxes
    ):
        ...
        for i_bch in range(batch_size):
            row_inds = (
                torch.nonzero(trk_correspondences[i_bch].sum(dim=1)).long().view(-1)
            )
            col_inds = (
                torch.nonzero(trk_correspondences[i_bch].sum(dim=0)).long().view(-1)
            )
            ...
            if row_inds.numel() > 0 and col_inds.numel() > 0:
                ...
                assert gt_rois_t0.size(0) == gt_rois_t1.size(
                    0
                ...
                tracking_labels_batch[i_bch] = batch_gt_rois_t0[i_bch][
                    :, 4
                ]  # uncomment this line!
        ...
        tracking_target_data = self._compute_targets_pytorch(
            batch_gt_rois_t0[:, :, :4], batch_gt_rois_t1[:, :, :4]
        )
        tracking_targets, tracking_inside_weights = self._get_tracking_regression_labels_pytorch(
            tracking_target_data, tracking_labels_batch, num_classes
        )
        # set tracking rois to gt rois in frame t0
        # tracking_rois_batch = gt_boxes[0][:,:,:5]
        return (
            tracking_labels_batch,
            tracking_rois_batch,
            tracking_targets,
            tracking_inside_weights,
        )
