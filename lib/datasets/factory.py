...
from datasets.imagenet_detect import imagenet_detect
...
for split in ["train", "val", "test"]:
    ...
    __sets[name] = lambda split=split, devkit_path=devkit_path: imagenet_detect(
        split, devkit_path, "VID"
    )

# set up image net det with classes from VID.
for split in ["train", "val", "test"]:
    name = "imagenet_det_{}".format(split)
    devkit_path = os.path.join("data", "ILSVRC")
    data_path = os.path.join("data", "ILSVRC")
    __sets[name] = lambda split=split, devkit_path=devkit_path: imagenet_detect(
        split, devkit_path, "DET"
    )
