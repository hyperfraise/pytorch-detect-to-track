...
class imagenet_detect(imdb):
    ...
    def _load_image_set_index(self):
        ...
        image_set_index_file = os.path.join(
            self._data_path, "ImageSets", self._det_vid, self._image_set + ".txt"
        )
    ...
    def gt_roidb(self):
        ...
        if os.path.exists(cache_file):
            ...
            return roidb
        gt_roidb = [
            self._load_vid_annotation(idx, index)
    ...
    def _load_vid_annotation(self, idx, index):
        ...
        if self._det_vid == "DET":
            filename = os.path.join(
                self._data_path, "Annotations", "DET", self._image_set, index + ".xml"
            )
        else:
            filename = os.path.join(
                self._data_path, "Annotations", "VID", self._image_set, index + ".xml"
            )
    ...
    def _do_python_eval(self, output_dir="output"):
        annopath = os.path.join(
            self._devkit_path, "Annotations", "VID", self._image_set, "{:s}.xml"
        )
        imagesetfile = os.path.join(
            self._devkit_path, "ImageSets", "VID", self._image_set + ".txt"
        )
    ...
    def evaluate_detections(self, all_boxes, roidb, output_dir):
        self._roidb = roidb
        self._image_index = [
            "/".join(roi_entry[0]["image"].split("/")[-3:])
            .replace(".JPEG", "")
            .replace(".jpeg", "")
            .replace(".jpg", "")
            .replace(".JPG", "")
