...
class DatasetParser:
    ...
    def init_paths(self):
        ...
        assert os.path.isdir(
            self.data_dir
        ), "A valid directory required. No directory named {}".format(self.data_dir)
        return
...
class ImagenetVID(DatasetParser):
    ...
    def write_to_file(self):
        for segment in self.vid_list:
            ...
            for video in self.vid_list[segment]:
                if len(self.vid_list[segment][video]) == 0:
                    continue
                ...
                for frame in self.vid_list[segment][video]:
                    ...
                    with open(fname, "a+") as f:
                        f.write(
                            os.path.join(segment, video, frame.split(".")[0])
                            + " 1"
                            + " "
                            + str(frame_number)
                            + " "
    ...
    def get_vid_list(self):
        ...
        for segment in os.listdir(self.dataset_path):
            if segment not in self.vid_list:
                continue
            ...
            for i, vid in enumerate(os.walk(seg_path)):
                ...
                if args.frames_per_video != -1:
                    frame_list = frame_list[
                        0 :: int(
                            np.ceil(len(frame_list) / float(args.frames_per_video))
                        )
                    ]
                    # consecutive frame
