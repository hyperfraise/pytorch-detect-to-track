...
if __name__ == "__main__":
    ...
    if args.use_det:
        ...
        # vid and det
        vid_imdb, vid_roidb, vid_ratio_list, vid_ratio_index = combined_roidb(
            imdb_names[0]
        )
        det_imdb, det_roidb, det_ratio_list, det_ratio_index = combined_roidb(
            imdb_names[1], duplicate_frames=True
        )
        vid_train_size, det_train_size = len(vid_roidb), len(det_roidb)
        train_size = min(vid_train_size, det_train_size)
        assert (
            vid_imdb.num_classes == det_imdb.num_classes
        ), "VID and DET must have same number of classes."
    ...
    if args.use_det:
        ...
        vid_dataset = roibatchLoader(
            vid_roidb,
            vid_ratio_list,
            vid_ratio_index,
            args.batch_size,
            vid_imdb.num_classes,
            training=True,
        )
        det_dataset = roibatchLoader(
            det_roidb,
            det_ratio_list,
            det_ratio_index,
            args.batch_size,
            det_imdb.num_classes,
            training=True,
        )
    ...
    if args.net == "res101":
        RFCN = resnet(
            imdb_classes, 101, pretrained_rfcn=True, class_agnostic=args.class_agnostic
        )
    elif args.net == "res50":
        RFCN = resnet(
            imdb_classes, 50, pretrained=True, class_agnostic=args.class_agnostic
        )
    elif args.net == "res152":
        RFCN = resnet(
            imdb_classes, 152, pretrained=True, class_agnostic=args.class_agnostic
        )
    else:
    ...
    for epoch in range(args.start_epoch, args.max_epochs):
        ...
        for step in range(iters_per_epoch):
            ...
            if step % args.disp_interval == 0:
                ...
                print(
                    "[session %d][epoch %2d][iter %4d] loss: %.4f, lr: %.2e"
                ...
                print(
                    "\t\t\trpn_cls: %.4f, rpn_box: %.4f, rcnn_cls: %.4f, rcnn_box %.4f trk_box %.4f"
                    % (
                        loss_rpn_cls,
                        loss_rpn_box,
                        loss_rcnn_cls,
        ...
        if args.mGPUs:
            save_name = os.path.join(
                output_dir,
                "rfcn_detect_track_{}_{}_{}.pth".format(args.session, epoch, step),
            )
            save_checkpoint(
                {
