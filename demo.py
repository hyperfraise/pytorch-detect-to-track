...
def parse_args():
  ...
  """
    ...
    parser.add_argument("--net", dest="net", help="res101", default="res101", type=str)
    parser.add_argument(
        "--set",
        dest="set_cfgs",
...
def visualize_without_paths(
    video_dataset, pred_boxes, scores, pred_trk_boxes, det_classes
):
    print("Visualizing...")
...
def visualize_with_paths(video_dataset, video_post_proc):
    ...
    for i_frame in range(num_frames):
        ...
        img = img[:, :, (2, 1, 0)]
        disp_image = Image.fromarray(np.uint8(img))
        for i_pth, cls_ind in enumerate(
            video_post_proc.path_labels
        ):  # iterate over path labels
            cls_ind = int(cls_ind)
            ax.imshow(disp_image, aspect="equal")
            class_name = det_classes[cls_ind]
            ...
            if (
                i_frame >= path_starts and i_frame <= path_ends
            ):  # is this frame in the current path
                # bboxes for this class path
                bbox = (
                    video_post_proc.path_boxes[i_pth][i_frame - path_starts]
                    .cpu()
                    .numpy()
                )
                # scores for this class path
                score = (
                    video_post_proc.path_scores[i_pth][i_frame - path_starts]
                    .cpu()
                    .numpy()
                )
...
if __name__ == "__main__":
    ...
    load_name = os.path.join(
        input_dir,
        "rfcn_detect_track_{}_{}_{}.pth".format(
    ...
    for ivid in range(len(video_dataset)):
        ...
        vid_pred_trk_boxes = (
            []
        )  # container for predicted tracking boxes over all frames
        ...
        for frames in vid_blob:
            ...
            rois, cls_prob, bbox_pred, tracking_pred, rpn_loss_cls, rpn_loss_box, RCNN_loss_cls, RCNN_loss_bbox, rois_label, tracking_loss_bbox = RFCN(
                ...
                if cfg.TRAIN.BBOX_NORMALIZE_TARGETS_PRECOMPUTED:
                    if args.class_agnostic:
                        ...
                        box_deltas = box_deltas.view(
                            n_legs, batch_size, -1, 4 * len(imagenet_vid_classes)
                        )
            ...
            print(
                "Processed frame pair : t={}, t+tau={} / {}".format(
                    curr_frame_t0, curr_frame_t1, video_dataset._n_frames - 1
                )
            )
        end_det = time.time()
        ...
        if len(vid_pred_boxes) == 0:
            ...
            vid_post_proc = VideoPostProcessor(
                vid_pred_boxes,
                vid_scores,
                vid_pred_trk_boxes,
                imagenet_vid_classes,
